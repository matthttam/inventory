...
class DeviceTest(TestCase):
    ...
    def test_asset_id_required(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'asset_id').blank, False)
        self.assertEqual(device._meta.get_field(
            'asset_id').null, False)
    ...
    def test_status_label(self):
        device = Device.objects.get(id=1)
        field_label = device._meta.get_field(
            'status').verbose_name
    ...
    def test_status_foreign_key(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'status').related_model, DeviceStatus)

    def test_google_id_label(self):
        device = Device.objects.get(id=1)
        field_label = device._meta.get_field(
            'google_id').verbose_name
    ...
    def test_google_location_optional(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'google_location').blank, True)
        self.assertEqual(device._meta.get_field(
            'google_location').null, False)

    def test_google_most_recent_user_label(self):
        device = Device.objects.get(id=1)
        field_label = device._meta.get_field(
            'google_most_recent_user').verbose_name
        self.assertEqual(field_label, 'google most recent user')

    def test_google_most_recent_user_max_length(self):
        device = Device.objects.get(id=1)
        max_length = device._meta.get_field(
            'google_most_recent_user').max_length
        self.assertEqual(max_length, 255)

    def test_google_most_recent_user_optional(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'google_most_recent_user').blank, True)
        self.assertEqual(device._meta.get_field(
            'google_most_recent_user').null, False)

    def test_device_model_foreign_key(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'device_model').related_model, DeviceModel)

    def test_device_model_required(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'device_model').blank, False)
        self.assertEqual(device._meta.get_field(
            'device_model').null, False)

    def test_building_foreign_key(self):
        device = Device.objects.get(id=1)
        self.assertEqual(device._meta.get_field(
            'building').related_model, Building)
    ...
    def test___str__(self):
        device = DeviceFactory(asset_id="test_asset_id",
                               serial_number="test_serial_number")
        self.assertEqual(
            device.__str__(), f"test_asset_id (test_serial_number) - {device.device_model}")
