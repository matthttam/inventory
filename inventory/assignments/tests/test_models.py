...
class AssignmentActions(TestCase):
    @patch("devices.models.device_assignment_creation_action")
    @patch("devices.models.device_turnin_action")
    ...
    def test_device_assignment_save_updates_device_building(self):
        ...
        self.assertNotEqual(device.building.id, person.primary_building.id)
        DeviceAssignmentFactory(device=device, person=person)
        self.assertEqual(device.building.id, person.primary_building.id)

    def test_device_assignment_create_with_no_primary_building_does_not_update_device(self):
        building_a = BuildingFactory(name="A")
        building_b = BuildingFactory(name="B")
        device = DeviceFactory(building=building_a)
        person = PersonFactory(primary_building=building_b)
...
class AssignmentChangeDeviceOU(self):
    pass